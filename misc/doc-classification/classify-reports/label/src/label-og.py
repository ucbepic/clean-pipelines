...
def preprocess_page_image(image: np.ndarray, size: tuple[int, int] = (212, 212)) -> np.ndarray:
    """
    Preprocess a page image with enhanced feature definition and larger size for better
    model performance.
...
def process_page_range(
    pdf_path, start_page, end_page, document_id, output_dir, label, size=(212, 212)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Processing {pdf_path} pages {start_page}-{end_page} (document_id: {document_id})"
        )
...
def parse_args():
    ...
    parser.add_argument("--gt-tbl", type=str, required=True, help="Path to groundtruth table CSV")
...
def main():
    ...
    if len(negatives) < needed_negatives:
        logger.info(
            f"Need {needed_negatives} negatives but only have {len(negatives)}. Will oversample."
        )
    ...
    if all_results:
        ...
        if neg_usage:
            ...
            logger.info(
                f"Times each negative PDF was used: min={min(usage_counts)}, "
                f"max={max(usage_counts)}, avg={sum(usage_counts) / len(usage_counts):.2f}"
            )
    else:
