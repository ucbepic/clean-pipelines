    ...
    def get_model(model_name, num_classes=2):
        ...
        with torch.no_grad():
            new_conv1.weight = nn.Parameter(
                base_model.conv1.weight.mean(dim=1, keepdim=True).repeat(1, 3, 1, 1)
            )
...
class ExperimentManager:
    def __init__(self, train_df, valid_df, test_df, device, base_output_dir, args):
        ...
        self.best_model_info = {"test_f1": float("-inf"), "params": None, "model_path": None}
    ...
    def run_single_experiment(self, params, exp_dir):
        ...
        train_loader, valid_loader = get_data_loaders(
            self.train_df, self.valid_df, batch_size=params["batch_size"]
        )
    ...
    def run_experiments(self):
        ...
        for i, params in enumerate(experiments, 1):
            ...
            try:
                test_metrics, model_path, test_predictions_path = self.run_single_experiment(
                    params, exp_dir
                )
                ...
                self.results.append({**params, **test_metrics})

                # Save updated results after each experiment
                results_df = pd.DataFrame(self.results)
                results_df.to_csv(
                    os.path.join(self.experiment_dir, "all_experiments_results.csv"), index=False
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, default="cuda", choices=["cpu", "cuda"], help="Device to use"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train-split", type=float, default=0.8, help="Train/validation split ratio"
    )
