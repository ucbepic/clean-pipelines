...
class DocumentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for _, row in doc_pages.iterrows():
            try:
                # Load as grayscale since preprocessing outputs grayscale images
...
def get_data_loaders(train_df, valid_df, batch_size=32):
    # Modified transforms to work with preprocessed images
    train_transform = transforms.Compose(
        [
            transforms.Resize((224, 224)),
            # Reduced augmentations since images are preprocessed
            transforms.RandomRotation(10),  # Reduced from 15
            transforms.RandomResizedCrop(224, scale=(0.9, 1.0)),  # Tightened scale
            # Removed ColorJitter since images are grayscale
            transforms.ToTensor(),
            # Adjusted normalization for preprocessed images
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),  # Changed from ImageNet stats
        ]
    )
...
class DocumentClassifier(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # Modified ResNet initialization for preprocessed images
        resnet = models.resnet34(pretrained=True)

        # Modify first conv layer to handle preprocessed images better
        ...
        with torch.no_grad():
            new_conv1.weight = nn.Parameter(
                resnet.conv1.weight.mean(dim=1, keepdim=True).repeat(1, 3, 1, 1)
            )

        resnet.conv1 = new_conv1

        # Freeze fewer layers since we've modified the input processing
        for param in list(resnet.children())[:-5]:  # Freeze one less block
        ...
        # Modified classifier for preprocessed features
        self.classifier = nn.Sequential(
            nn.Linear(feature_dim, 256),  # Increased from 128
            nn.ReLU(),
...
#         # Modified ResNet initialization for preprocessed images
#         resnet = models.resnet50(pretrained=True)  # Changed to ResNet50
...
#         # Updated feature_dim for ResNet50
#         feature_dim = 2048  # Changed from 512
...
#         # Modified ResNet initialization for preprocessed images
#         resnet = models.resnet101(pretrained=True)  # Changed to ResNet101
...
#         # Feature dimension remains 2048 for ResNet101 (same as ResNet50)
#         feature_dim = 2048
...
#         # Modified ResNet initialization for preprocessed images
#         resnet = models.resnet152(pretrained=True)  # Changed to ResNet152
...
#         # Feature dimension remains 2048 for ResNet152 (same as ResNet50/101)
#         feature_dim = 2048
...
def main(args):
    ...
    train_ids, temp_ids = train_test_split(
        doc_ids, train_size=args.train_split, random_state=args.seed
    )
    valid_ids, test_ids = train_test_split(temp_ids, test_size=0.5, random_state=args.seed)
    ...
    best_model_path = train_model(
        model, train_loader, valid_loader, args.epochs, device, args.model_dir
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train-split", type=float, default=0.6, help="Proportion of data to use for training"
    )
