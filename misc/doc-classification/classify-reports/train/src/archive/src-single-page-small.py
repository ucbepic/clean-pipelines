...
def main(args):
    ...
    doc_level_dist = df.groupby("document_id")["label"].first().value_counts()
    ...
    doc_labels = df.groupby("document_id")["label"].first()

    # Split on document level with stratification
    train_ids, temp_ids = train_test_split(
        doc_ids, train_size=args.train_split, random_state=args.seed, stratify=doc_labels
    )
    valid_ids, test_ids = train_test_split(
        temp_ids, test_size=0.5, random_state=args.seed, stratify=doc_labels[temp_ids]
    )
    ...
    train_doc_dist = train_df.groupby("document_id")["label"].first().value_counts()
    ...
    valid_doc_dist = valid_df.groupby("document_id")["label"].first().value_counts()
    ...
    test_doc_dist = test_df.groupby("document_id")["label"].first().value_counts()
    ...
    if "document_id" in test_df.columns:
        results_df = pd.merge(
            results_df, test_df[["img_filepath", "document_id"]], on="img_filepath", how="left"
        )
    ...
    precision = precision_score(
        results_df["true_label"], results_df["predicted_label"], average="weighted"
    )
    recall = recall_score(
        results_df["true_label"], results_df["predicted_label"], average="weighted"
    )
