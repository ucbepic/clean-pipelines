...
def perform_entity_resolution_excel(incident_df, post_df, output_path="entity_matches.xlsx"):
    ...
    weights = {"first_name": 0.45, "last_name": 0.45, "middle_name": 0.5, "suffix": 0.5}
    ...
    incident_df["incident_date"] = pd.to_datetime(incident_df["incident_date"]).dt.strftime(
        "%Y-%m-%d"
    )
    ...
    fresno_post = post_df[
        post_df["agency_name"].str.contains("|".join(target_agencies), case=False, na=False)
    ]
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        for idx, incident in incident_df.iterrows():
            ...
            for _, post in relevant_post.iterrows():
                ...
                similarities = {
                    "first_name_sim": calculate_name_similarity(
                        incident["first_name"], post["first_name"], "first_name"
                    ),
                    "last_name_sim": calculate_name_similarity(
                        incident["last_name"], post["last_name"], "last_name"
                    ),
                    "middle_name_sim": calculate_name_similarity(
                        incident["middle_name"], post["middle_name"], "middle_name"
                    ),
                    "suffix_sim": calculate_name_similarity(
                        incident["suffix"], post["suffix"], "suffix"
                    ),
                }
                ...
                candidate_data = {
                    ...
                    "incident_name": (
                        f"{incident['first_name']} {incident['middle_name']} "
                        f"{incident['last_name']} {incident['suffix']}"
                    ).strip(),
                    ...
                    "post_name": (
                        f"{post['first_name']} {post['middle_name']} "
                        f"{post['last_name']} {post['suffix']}"
                    ).strip(),
            ...
            sheet_name = (
                f"ID_{str(incident_uid)[:28]}"  # Excel has a 31 character limit for sheet names
            )
            ...
            for row in range(2, 5):  # Rows 2-4 (1-based)
                score_cell = worksheet.cell(
                    row=row, column=display_columns.index("similarity_score") + 1
                )
...
def main():
    ...
    matches_output = perform_entity_resolution_excel(
        incident_df=incident_df, post_df=post_df, output_path=args.output
    )
    ...
    training_df, complete_df = create_training_data(
        all_matches, args.training_output, all_matches_path
    )
