...
def engineer_name_features(df):
    ...
    for name_part, (incident_col, post_col) in name_columns.items():
        ...
        string_similarities = df.apply(
            lambda x: calculate_string_similarity(
                x[incident_col], 
                x[post_col]
            ),
            axis=1
        ...
        features.update({
            ...
            f'{name_part}_name_levenshtein': [x['levenshtein_norm'] for x in string_similarities],
    ...
    df['incident_full_name'] = df.apply(
        lambda x: ' '.join(filter(None, [
            ...
            str(x['incident_middle_name']) if not pd.isna(x['incident_middle_name']) else '',
...
def normalize_features(df):
    ...
    features_to_scale = [col for col in df.columns if any(x in col for x in 
        ['jaro', 'levenshtein', 'length_ratio', 'embedding'])]
