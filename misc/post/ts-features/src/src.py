...
def engineer_name_features(df):
    ...
    for name_part, (incident_col, post_col) in name_columns.items():
        ...
        string_similarities = df.apply(
            lambda x, incident_col=incident_col, post_col=post_col: calculate_string_similarity(
                x[incident_col], x[post_col]
            ),
            axis=1,
        )
        ...
        features[f"{name_part}_name_embedding"] = df.apply(
            lambda x, incident_col=incident_col, post_col=post_col: calculate_embedding_similarity(
                x[incident_col], x[post_col], model
            ),
            axis=1,
        )
