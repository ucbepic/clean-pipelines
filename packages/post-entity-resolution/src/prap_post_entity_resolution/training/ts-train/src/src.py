...
def compare_models(X_train, X_test, y_train, y_test, model_types=None):
    """Train and compare the requested models (default: all), returning the best one."""
    models = {
    ...
    for model_name, result in results.items():
        ...
        print(
            f"Precision (class 1): {result['report'].get('1', result['report'].get(1, {})).get('precision', 0):.4f}"
        )
        print(
            f"Recall (class 1): {result['report'].get('1', result['report'].get(1, {})).get('recall', 0):.4f}"
        )
        print(
            f"F1-score (class 1): {result['report'].get('1', result['report'].get(1, {})).get('f1-score', 0):.4f}"
        )
...
def main():
    ...
    selected = [args.model_type] if args.model_type else None
    best_model, best_model_name, results = compare_models(
        X_train, X_test, y_train, y_test, model_types=selected
    )
