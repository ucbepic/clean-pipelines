...
def parse_args():
    ...
    parser.add_argument(
        "--json-input", required=True, help="Path to input JSON file containing Fresno data"
    )
    parser.add_argument(
        "--csv-input", required=True, help="Path to input CSV file containing POST data"
    )
...
def read_json(json_path):
    ...
    df = df[
        ["first_name", "middle_name", "last_name", "suffix", "agency_name", "incident_date", "uid"]
    ]
...
def filter_for_fresno_agencies(df):
    ...
    target_persons = df[df["agency_name"].str.contains(pattern, case=False, na=False)][
        "person_nbr"
    ].unique()
...
def read_post(csv_path):
    ...
    df = df.pipe(clean_post).pipe(filter_for_fresno_agencies)
