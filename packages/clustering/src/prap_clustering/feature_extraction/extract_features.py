...
def main():
    ...
    parser.add_argument("input_csv", help="Path to input CSV with OCR text and metadata")
    parser.add_argument(
        "-o",
        "--output",
        ...
        try:
            ...
            if "sha1" in df.columns and "sha1" in df_existing.columns:
                ...
                if merge_cols:
                    # Merge on sha1
                    df = df.drop(
                        columns=[col for col in merge_cols if col in df.columns], errors="ignore"
                    )
                    df = df.merge(
                        df_existing[["sha1"] + merge_cols],
                        on="sha1",
                        how="left",
                        suffixes=("", "_existing"),
                    )
