...
def evaluate_clusters(groundtruth_df: pd.DataFrame, test_df: pd.DataFrame) -> dict:
    ...
    if cluster_column is None:
        raise ValueError(
            f"Could not find cluster column. Available columns: {list(test_df.columns)}"
        )
    ...
    for gt_cluster_name, group in groundtruth_df.groupby("provisional_case_name"):
        ...
        if files_found_in_test == 0:
            print(
                f"Warning: No files from groundtruth cluster '{gt_cluster_name}' found in test results"
            )
            continue

        # Find the test cluster with the most files from this groundtruth cluster
        best_test_cluster = max(
            test_cluster_distribution.items(), key=lambda x: len(x[1]), default=(None, set())
        )
        ...
        if best_cluster_id is not None:
            ...
            total_in_best_cluster = sum(
                1 for f, c in file_to_test_cluster.items() if c == best_cluster_id
            )

            # Metrics
            precision = (
                files_in_best_cluster / total_in_best_cluster if total_in_best_cluster > 0 else 0
            )
            ...
            merged_files = [
                f
                for f, c in file_to_test_cluster.items()
            ...
            cluster_metrics.append(
                {
                    ...
                    "split_distribution": {
                        str(cluster): list(files) for cluster, files in split_files.items()
                    },
    ...
    if cluster_metrics:
        ...
        clusters_with_merges = sum(
            1 for m in cluster_metrics if m["num_incorrectly_merged_files"] > 0
        )
    else:
...
def compute_bcubed_metrics(groundtruth_df: pd.DataFrame, file_to_test_cluster: dict) -> dict:
    ...
    file_to_gt_cluster = dict(
        zip(groundtruth_df["gdrive_name"], groundtruth_df["provisional_case_name"], strict=False)
    )
    ...
    if bcubed_precisions and bcubed_recalls:
        ...
        if (avg_bcubed_precision + avg_bcubed_recall) > 0:
            avg_bcubed_f1 = (
                2
                * (avg_bcubed_precision * avg_bcubed_recall)
                / (avg_bcubed_precision + avg_bcubed_recall)
            )
        else:
            avg_bcubed_f1 = 0.0
