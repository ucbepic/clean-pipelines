...
OUTPUT_PATHS = {
    (
        "filepath_only",
        False,
    ): "../../data/output/test_clustering_results_graph_filepath_only_no_validation.csv",
    (
        "filepath_only",
        True,
    ): "../../data/output/test_clustering_results_graph_filepath_only_with_validation.csv",
    (
        "filename_only",
        False,
    ): "../../data/output/test_clustering_results_graph_filename_only_no_validation.csv",
    (
        "filename_only",
        True,
    ): "../../data/output/test_clustering_results_graph_filename_only_with_validation.csv",
    (
        "combined",
        False,
    ): "../../data/output/test_clustering_results_graph_combined_no_validation.csv",
    (
        "combined",
        True,
    ): "../../data/output/test_clustering_results_graph_combined_with_validation.csv",
}
...
def process_pairs_in_batches(
    pairs: list[tuple], max_workers: int, batch_size: int, source_mode: str
):
    """
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for batch_idx in range(0, len(pairs), batch_size):
            ...
            logger.info(
                f"[Batch {batch_num}/{total_batches}] Processing {len(batch_pairs):,} pairs..."
            )
            ...
            logger.info(
                f"[Batch {batch_num}/{total_batches}] Complete: {len(valid_results):,} edges found in {batch_elapsed:.1f}s"
            )
            logger.info(
                f"[Progress] {progress_pct:.1f}% | Edges: {total_edges:,} | Elapsed: {total_elapsed / 60:.1f}m | ETA: {eta_minutes:.1f}m"
            )
            logger.info(f"{'=' * 80}")
...
def cluster_documents(
    data: list[dict], source_mode: str, enable_validation: bool
) -> dict[int, int]:
    """
    ...
    logger.info(
        f"  ℹ Edge density: {total_edges_added / len(doc_pairs) * 100:.2f}% of pairs matched"
    )
    ...
    logger.info(
        f"  ℹ Singleton clusters: {singletons:,} ({singletons / len(candidate_clusters) * 100:.1f}%)"
    )
    ...
    if enable_validation:
        ...
        logger.info(
            f"  Threshold: {VALIDATION_THRESHOLD * 100}% (each node must connect to ≥{VALIDATION_THRESHOLD * 100}% of cluster)"
        )
        validation_start = time.time()

        validated_clusters = validate_and_split_clusters(
            candidate_clusters, G, threshold=VALIDATION_THRESHOLD, debug=DEBUG
        )
...
def run_single_variation(
    data: list[dict], feature_source: str, enable_validation: bool, output_path: str
):
    ...
    logger.info(
        f"Singleton clusters: {sum(1 for cid in node_to_cluster.values() if list(node_to_cluster.values()).count(cid) == 1)}"
    )
    logger.info(
        f"Largest cluster size: {max(list(node_to_cluster.values()).count(cid) for cid in set(node_to_cluster.values()))}"
    )
...
def main():
    ...
    if not feature_sources:
        logger.error(
            "No feature sources enabled! Set at least one RUN_*_ONLY or RUN_COMBINED flag to True."
        )
        return

    if not validation_settings:
        logger.error(
            "No validation settings enabled! Set at least one RUN_VALIDATION_* flag to True."
        )
        return
    ...
    for idx, (feature_source, enable_validation) in enumerate(variations, 1):
        logger.info(f"\n{'#' * 80}")
        logger.info(
            f"# VARIATION {idx}/{len(variations)}: {feature_source} + validation={enable_validation}"
        )
        logger.info(f"{'#' * 80}")
