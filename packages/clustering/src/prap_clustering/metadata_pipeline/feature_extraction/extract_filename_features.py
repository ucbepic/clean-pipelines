...
def print_coverage_stats(stats: dict):
    ...
    logger.info("\nDates:")
    logger.info(f"  From directory path (fp): {stats['documents_with_dates_fp']} ({stats['documents_with_dates_fp']/total*100:.1f}%)")
    logger.info(f"  From filename (fn): {stats['documents_with_dates_fn']} ({stats['documents_with_dates_fn']/total*100:.1f}%)")
    logger.info(f"  From either: {stats['documents_with_dates_any']} ({stats['documents_with_dates_any']/total*100:.1f}%)")
    logger.info("\nCase IDs:")
    logger.info(f"  From directory path (fp): {stats['documents_with_case_ids_fp']} ({stats['documents_with_case_ids_fp']/total*100:.1f}%)")
    logger.info(f"  From filename (fn): {stats['documents_with_case_ids_fn']} ({stats['documents_with_case_ids_fn']/total*100:.1f}%)")
    logger.info(f"  From either: {stats['documents_with_case_ids_any']} ({stats['documents_with_case_ids_any']/total*100:.1f}%)")
    logger.info("\nNames:")
    logger.info(f"  From directory path (fp): {stats['documents_with_names_fp']} ({stats['documents_with_names_fp']/total*100:.1f}%)")
    logger.info(f"  From filename (fn): {stats['documents_with_names_fn']} ({stats['documents_with_names_fn']/total*100:.1f}%)")
    logger.info(f"  From either: {stats['documents_with_names_any']} ({stats['documents_with_names_any']/total*100:.1f}%)")
    logger.info("\nOverall:")
    logger.info(f"  Documents with ANY feature: {stats['documents_with_any_feature']} ({stats['documents_with_any_feature']/total*100:.1f}%)")
    logger.info(f"  Documents with NO features: {stats['documents_with_no_features']} ({stats['documents_with_no_features']/total*100:.1f}%)")
    logger.info(f"{'='*80}")
...
def main():
    ...
    feature_cols = ['extracted_dates_fp', 'extracted_dates_fn', 'extracted_case_ids_fp',
                   'extracted_case_ids_fn', 'extracted_names_fp', 'extracted_names_fn']
