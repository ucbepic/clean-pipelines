...
def score(
    ...
    cluster_column = next(
        (c for c in ["Parent Clusters", "Clusters", "cluster", "parent_clusters"] if c in df.columns),
        "",
    )
...
ABLATION_CONFIG_PATH_V2 = Path(
    _os.environ.get("PRAP_CLUSTERING_ABLATION_CONFIG_V2", _CONFIGS_DIR / "ablation_configs_handengineered.yaml")
)
...
ABLATION_OUTPUT_DIR_V2 = Path(
    _os.environ.get("PRAP_CLUSTERING_ABLATIONS_V2_DIR", Path.cwd() / "data" / "output" / "ablations_v2_dir_fallback")
)

# Directory containing ML ablation clustering results (learned rules)
ABLATION_OUTPUT_DIR_ML = Path(
    _os.environ.get("PRAP_CLUSTERING_ABLATIONS_ML_DIR", Path.cwd() / "data" / "output" / "ablations_ml")
)
...
def discover_ablation_results_for_agency(agency_name: str, output_dir: Path, config_path: Path, version: str = 'v2') -> list[dict]:
    ...
    for file_path in sorted(result_files):
        ...
        if version == 'embeddings':
            ...
            is_old_learned_rule = ablation_name in ["learned_rules_decision_tree", "learned_rules_random_forest"]
        ...
        configs.append({
            'input_csv': file_path,
            'output_report': str(REPORTS_DIR / agency_name / f"{version}_{ablation_name}_report.txt"),
...
def print_cluster_analysis(metrics: dict, method_name: str, input_file: str, output_file=None):
    ...
    output(f"  Perfect (F1 = 1.0):           {summary['perfect_clusters']:3d} ({summary['perfect_clusters']/total_clusters*100:.1f}%)")
    output(f"  High quality (0.9 ≤ F1 < 1.0): {summary['high_quality_clusters']:3d} ({summary['high_quality_clusters']/total_clusters*100:.1f}%)")
    output(f"  Medium quality (0.7 ≤ F1 < 0.9): {summary['medium_quality_clusters']:3d} ({summary['medium_quality_clusters']/total_clusters*100:.1f}%)")
    output(f"  Low quality (F1 < 0.7):       {summary['low_quality_clusters']:3d} ({summary['low_quality_clusters']/total_clusters*100:.1f}%)")
    output()
    ...
    output("-" * 80)
    output(f"Clusters with splits:  {overall['clusters_with_splits']} ({overall['clusters_with_splits']/total_clusters*100:.1f}%)")
    output(f"Clusters with merges:  {overall['clusters_with_merges']} ({overall['clusters_with_merges']/total_clusters*100:.1f}%)")
    output()
    ...
    for i, cluster in enumerate(cluster_details, 1):
        ...
        output(f"   F1 Score:    {cluster['f1']:.4f} (Precision: {cluster['precision']:.4f}, Recall: {cluster['recall']:.4f})")
        output(f"   Should have: {cluster['num_files_should_be_together']} documents")
        output(f"   Best match:  {cluster['num_files_in_best_cluster']} documents in test cluster {cluster['best_matching_test_cluster']}")
        ...
        if cluster['num_incorrectly_merged_files'] > 0:
            output(f"   Merged with: {cluster['num_incorrectly_merged_files']} documents from other ground truth clusters")
...
def generate_comparison_report(all_metrics: list, agency_name: str, output_path: str = None):
    ...
    with open(output_path, 'w') as f:
        ...
        for rank, result in enumerate(sorted_results, 1):
            ...
            if config:
                ...
                if use_learned or use_cascade:
                    ...
                    output(f"   Config: Date Window={date_window}, Require Dates={require_dates}, Tiers={tiers_str}")

            output(f"   Precision: {overall['precision']:.4f} ({overall['precision']*100:.2f}%)")
            output(f"   Recall:    {overall['recall']:.4f} ({overall['recall']*100:.2f}%)")
            output(f"   F1 Score:  {overall['f1']:.4f} ({overall['f1']*100:.2f}%)")
            output(f"   Clusters evaluated: {overall['num_groundtruth_clusters_evaluated']}")
            output(f"   Splits: {overall['clusters_with_splits']} ({overall['clusters_with_splits']/overall['num_groundtruth_clusters_evaluated']*100:.1f}%)")
            output(f"   Merges: {overall['clusters_with_merges']} ({overall['clusters_with_merges']/overall['num_groundtruth_clusters_evaluated']*100:.1f}%)")
            output()
        ...
        output("=" * 80)
        output()
        output(f"{'Ablation':<30} {'Date':>6} {'ReqD':>4} {'Tiers':>6} {'Prec':>6} {'Rec':>6} {'F1':>6} {'Split':>5} {'Merge':>5}")
        output("-" * 80)
        ...
        for result in sorted_results:
            ...
            output(f"{display_name:<30} {date_str:>6} {require_dates:>4} {tiers_str:>6} "
...
def print_category_statistics_table(ablation_averages: list[dict], category_name: str, output_func):
    ...
    relevant_ablations = [a for a in ablation_averages if a.get(category_key) and len(a[category_key]) > 0]
    ...
    output_func("=" * 100)
    output_func()
    output_func("This table shows statistics for each ablation across all agencies in this category.")
    ...
    output_func("-" * 115)
    output_func(f"{'Ablation':<30} {'Mean':>7} {'Median':>7} {'Std':>7} {'Var':>7} {'Min':>7} {'Max':>7} {'IQR':>7} {'CI_Lo':>7} {'CI_Hi':>7}")
    output_func("-" * 115)
...
def generate_cross_agency_summary(all_results: dict[str, list[dict]], output_path: str = None):
    ...
    with open(output_path, 'w') as f:
        ...
        output(f"  - Original evaluation agencies: {len(original_agencies)} (explicitly defined, used for V2 hand-coded rules tuning)")
        output(f"  - Learned rules training agencies: {len(learned_rules_agencies)} (used to train ML models, excluded from original)")
        output(f"  - Holdout test agencies: {len(holdout_agencies)} (true test set)")
        if uncategorized_agencies:
            output(f"  - Uncategorized agencies: {len(uncategorized_agencies)} (WARNING: not in any predefined list)")
        ...
        for method_name, agency_results in ablation_summary.items():
            ...
            avg_precision = sum(r['metrics']['precision'] for r in agency_results) / len(agency_results)
            ...
            if original_results:
                orig_avg_precision = sum(r['metrics']['precision'] for r in original_results) / len(original_results)
                orig_avg_recall = sum(r['metrics']['recall'] for r in original_results) / len(original_results)
                orig_avg_f1 = sum(r['metrics']['f1'] for r in original_results) / len(original_results)
            else:
                orig_avg_precision = orig_avg_recall = orig_avg_f1 = None

            # Calculate averages for HOLDOUT agencies
            if holdout_results:
                hold_avg_precision = sum(r['metrics']['precision'] for r in holdout_results) / len(holdout_results)
                hold_avg_recall = sum(r['metrics']['recall'] for r in holdout_results) / len(holdout_results)
                hold_avg_f1 = sum(r['metrics']['f1'] for r in holdout_results) / len(holdout_results)
            else:
            ...
            if learned_rules_results:
                learned_avg_precision = sum(r['metrics']['precision'] for r in learned_rules_results) / len(learned_rules_results)
                learned_avg_recall = sum(r['metrics']['recall'] for r in learned_rules_results) / len(learned_rules_results)
                learned_avg_f1 = sum(r['metrics']['f1'] for r in learned_rules_results) / len(learned_rules_results)
            else:
                learned_avg_precision = learned_avg_recall = learned_avg_f1 = None

            # Calculate B-Cubed averages for ORIGINAL agencies
            if original_results:
                orig_avg_bcubed_precision = sum(r['metrics'].get('bcubed_precision', 0) for r in original_results) / len(original_results)
                orig_avg_bcubed_recall = sum(r['metrics'].get('bcubed_recall', 0) for r in original_results) / len(original_results)
                orig_avg_bcubed_f1 = sum(r['metrics'].get('bcubed_f1', 0) for r in original_results) / len(original_results)
            else:
                orig_avg_bcubed_precision = orig_avg_bcubed_recall = orig_avg_bcubed_f1 = None

            # Calculate B-Cubed averages for HOLDOUT agencies
            if holdout_results:
                hold_avg_bcubed_precision = sum(r['metrics'].get('bcubed_precision', 0) for r in holdout_results) / len(holdout_results)
                hold_avg_bcubed_recall = sum(r['metrics'].get('bcubed_recall', 0) for r in holdout_results) / len(holdout_results)
                hold_avg_bcubed_f1 = sum(r['metrics'].get('bcubed_f1', 0) for r in holdout_results) / len(holdout_results)
            else:
            ...
            if learned_rules_results:
                learned_avg_bcubed_precision = sum(r['metrics'].get('bcubed_precision', 0) for r in learned_rules_results) / len(learned_rules_results)
                learned_avg_bcubed_recall = sum(r['metrics'].get('bcubed_recall', 0) for r in learned_rules_results) / len(learned_rules_results)
                learned_avg_bcubed_f1 = sum(r['metrics'].get('bcubed_f1', 0) for r in learned_rules_results) / len(learned_rules_results)
            else:
                learned_avg_bcubed_precision = learned_avg_bcubed_recall = learned_avg_bcubed_f1 = None

            # Calculate ARI averages for ORIGINAL agencies
            if original_results:
                ari_values = [r['metrics'].get('ari') for r in original_results if r['metrics'].get('ari') is not None]
            ...
            if holdout_results:
                ari_values = [r['metrics'].get('ari') for r in holdout_results if r['metrics'].get('ari') is not None]
            ...
            if learned_rules_results:
                ari_values = [r['metrics'].get('ari') for r in learned_rules_results if r['metrics'].get('ari') is not None]
        ...
        ablation_averages.sort(key=lambda x: x['orig_avg_f1'] if x['orig_avg_f1'] is not None else 0, reverse=True)
        ...
        output("=" * 80)
        output(f"Explicitly defined agencies used for V2 hand-coded rules tuning: {len(original_agencies)}")
        output()
        ...
        for rank, ablation in enumerate(ablation_averages, 1):
            ...
            output(f"   Average Precision: {ablation['orig_avg_precision']:.4f} ({ablation['orig_avg_precision']*100:.2f}%)")
            output(f"   Average Recall:    {ablation['orig_avg_recall']:.4f} ({ablation['orig_avg_recall']*100:.2f}%)")
            output(f"   Average F1 Score:  {ablation['orig_avg_f1']:.4f} ({ablation['orig_avg_f1']*100:.2f}%)")
            output()

            # Show per-agency breakdown for ORIGINAL agencies only
            output("   Per-agency results:")
            for result in sorted(ablation['original_results'], key=lambda x: x['metrics']['f1'], reverse=True):
                agency = result['agency']
                metrics = result['metrics']
                output(f"      {agency:<40s} F1={metrics['f1']:.4f}  P={metrics['precision']:.4f}  R={metrics['recall']:.4f}")
            output()
        ...
        output()
        output("NOTE: These agencies were used for manual feature engineering and ML model training.")
        ...
        ablation_averages_learned.sort(key=lambda x: x['learned_avg_f1'] if x['learned_avg_f1'] is not None else 0, reverse=True)
        ...
        for rank, ablation in enumerate(ablation_averages_learned, 1):
            ...
            output(f"   Average Precision: {ablation['learned_avg_precision']:.4f} ({ablation['learned_avg_precision']*100:.2f}%)")
            output(f"   Average Recall:    {ablation['learned_avg_recall']:.4f} ({ablation['learned_avg_recall']*100:.2f}%)")
            output(f"   Average F1 Score:  {ablation['learned_avg_f1']:.4f} ({ablation['learned_avg_f1']*100:.2f}%)")
            output()

            # Show per-agency breakdown for LEARNED RULES agencies only
            output("   Per-agency results:")
            for result in sorted(ablation['learned_rules_results'], key=lambda x: x['metrics']['f1'], reverse=True):
                agency = result['agency']
                metrics = result['metrics']
                output(f"      {agency:<40s} F1={metrics['f1']:.4f}  P={metrics['precision']:.4f}  R={metrics['recall']:.4f}")
            output()
        ...
        ablation_averages_holdout.sort(key=lambda x: x['hold_avg_f1'] if x['hold_avg_f1'] is not None else 0, reverse=True)
        ...
        for rank, ablation in enumerate(ablation_averages_holdout, 1):
            ...
            output(f"   Average Precision: {ablation['hold_avg_precision']:.4f} ({ablation['hold_avg_precision']*100:.2f}%)")
            output(f"   Average Recall:    {ablation['hold_avg_recall']:.4f} ({ablation['hold_avg_recall']*100:.2f}%)")
            output(f"   Average F1 Score:  {ablation['hold_avg_f1']:.4f} ({ablation['hold_avg_f1']*100:.2f}%)")
            output()

            # Show per-agency breakdown for HOLDOUT agencies only
            output("   Per-agency results:")
            for result in sorted(ablation['holdout_results'], key=lambda x: x['metrics']['f1'], reverse=True):
                agency = result['agency']
                metrics = result['metrics']
                output(f"      {agency:<40s} F1={metrics['f1']:.4f}  P={metrics['precision']:.4f}  R={metrics['recall']:.4f}")
            output()
        ...
        output("=" * 80)
        output()
        output("Comparison of performance on original agencies (hand-coded rules tuning) vs holdout agencies (unseen test data).")
        output()
        output(f"Original agencies: {len(original_agencies)} (explicitly defined, used for V2 hand-coded rules tuning)")
        output(f"Holdout agencies:  {len(holdout_agencies)} (true test set, never seen during tuning)")
        output()
        output(f"NOTE: Learned rules training agencies ({len(learned_rules_agencies)}) are excluded from this comparison.")
        ...
        output("Negative delta indicates worse performance on holdout (possible overfitting to tuning set).")
        output()
        ...
        for ablation in ablation_averages:
            if ablation['num_original'] > 0 and ablation['num_holdout'] > 0:
                ...
                output(f"{display_name:<30} {orig_f1:>8.4f} {hold_f1:>8.4f} {delta_str:>8} {pct_str:>10}")
        ...
        if comparison_data:
            ...
            output(f"Ablations with better holdout performance: {positive_deltas}/{len(comparison_data)}")
            output(f"Ablations with worse holdout performance:  {negative_deltas}/{len(comparison_data)}")
            output(f"Ablations with equal performance:         {neutral_deltas}/{len(comparison_data)}")
            output()
            ...
            output(f"  Delta: {best_generalizer['delta']:+.4f} ({best_generalizer['pct_change']:+.2f}%)")
            output()
        ...
        if original_agencies:
            ...
            for ablation in ablation_averages:
                ...
                agency_f1_map = {r['agency']: r['metrics']['f1'] for r in ablation['original_results']}
        ...
        if learned_rules_agencies and ablation_averages_learned:
            ...
            for ablation in ablation_averages_learned:
                ...
                agency_f1_map = {r['agency']: r['metrics']['f1'] for r in ablation['learned_rules_results']}
                ...
                mean_f1 = ablation['learned_avg_f1'] if ablation['learned_avg_f1'] is not None else 0
                row += f" {mean_f1:>7.4f}"
                csv_row['Mean'] = mean_f1
            ...
            if learned_csv_rows:
                ...
                for ablation in ablation_averages_learned:
                    ...
                    prec_vals = [r['metrics']['precision'] for r in ablation['learned_rules_results']]
        ...
        if holdout_agencies and ablation_averages_holdout:
            ...
            for ablation in ablation_averages_holdout:
                ...
                agency_f1_map = {r['agency']: r['metrics']['f1'] for r in ablation['holdout_results']}
        ...
        if original_agencies:
            ...
            for ablation in ablation_bcubed_sorted:
                ...
                agency_bcubed_map = {r['agency']: r['metrics'].get('bcubed_f1', 0) for r in ablation['original_results']}
        ...
        if holdout_agencies:
            ...
            for ablation in ablation_bcubed_sorted:
                ...
                # Build map of agency -> B-Cubed F1
                agency_bcubed_map = {r['agency']: r['metrics'].get('bcubed_f1', 0) for r in ablation['holdout_results']}
        ...
        if original_agencies:
            ...
            for ablation in ablation_ari_sorted:
                ...
                agency_ari_map = {r['agency']: r['metrics'].get('ari') for r in ablation['original_results']}
        ...
        if holdout_agencies:
            ...
            for ablation in ablation_ari_sorted:
                ...
                # Build map of agency -> ARI
                agency_ari_map = {r['agency']: r['metrics'].get('ari') for r in ablation['holdout_results']}
        ...
        output(f"{'Ablation':<30} {'B³-Prec':>8} {'B³-Rec':>8} {'B³-F1':>8} {'Macro-F1':>9} {'Δ':>7}")
        output("-" * 80)
        ...
        for ablation in ablation_bcubed_orig:
            ...
            output(f"{display_name:<30} {bcubed_prec:>8.4f} {bcubed_rec:>8.4f} {bcubed_f1:>8.4f} {macro_f1:>9.4f} {delta:>+7.4f}")
        ...
        ablation_ari_orig.sort(key=lambda x: x.get('orig_avg_ari', -1) if x.get('orig_avg_ari') is not None else -1, reverse=True)
        ...
        ablation_ari_learned.sort(key=lambda x: x.get('learned_avg_ari', -1) if x.get('learned_avg_ari') is not None else -1, reverse=True)
        ...
        ablation_ari_hold.sort(key=lambda x: x.get('hold_avg_ari', -1) if x.get('hold_avg_ari') is not None else -1, reverse=True)
...
def main():
    ...
    print(f"Found {len(agencies)} agencies total ({len(ml_agencies)} with ML results, {len(v2_agencies)} with V2/embeddings results):")
    ...
    for agency_idx, agency_name in enumerate(agencies, 1):
        ...
        ml_configs = discover_ablation_results_for_agency(
            agency_name,
            ABLATION_OUTPUT_DIR_ML,
            ABLATION_CONFIG_PATH_ML,
            version='ml'
        )
        ...
        if embeddings_configs:
            print(f"Found {len(embeddings_configs)} embeddings ablation result(s) for {agency_name}")
        else:
