...
def _clean(text: str) -> str:
    ...
    text = re.sub(
        r"(?i)(?:RESPONDING|INVESTIGATING) AGENCIES:\s*\n\s*No (?:responding|investigating) agencies identified on this page\.?\s*\n?",
        "",
        text,
    )
    text = re.sub(
    ...
    text = re.sub(
        r"(?i)No other agencies are explicitly named or described with investigative or responding roles on this page\.?\s*\n?",
        "",
        text,
    )
...
def filter_important_summaries(
    ...
    try:
        ...
        if total_summaries <= final_count:
            if logger:
                logger.info(
                    f"Case {case_name} (Row {row_index}): Only {total_summaries} summaries available, skipping filtering"
                )
            return summaries

        if total_summaries <= chunk_size:
            return filter_chunk(
                llm, summaries, final_count, logger, case_name, row_index
            )

        if logger:
            logger.info(
                f"Case {case_name} (Row {row_index}): Processing {total_summaries} summaries in chunks of {chunk_size}"
            )
        ...
        for i in range(num_chunks):
            ...
            if logger:
                logger.info(
                    f"Case {case_name} (Row {row_index}): Processing chunk {i+1}/{num_chunks} with {len(chunk)} summaries"
                )
        ...
        if len(important_from_chunks) > max_allowed_summaries:
            if logger:
                logger.info(
                    f"Case {case_name} (Row {row_index}): Filtering combined results from {len(important_from_chunks)} to {max_allowed_summaries}"
                )
        ...
        if logger:
            ...
            filtered_total_tokens = sum(
                len(encoding.encode(s)) for s in final_important
            )
            logger.info(
                f"Case {case_name} (Row {row_index}): Reduced from {original_total_tokens} to {filtered_total_tokens} tokens ({filtered_total_tokens/original_total_tokens:.2%} of original)"
            )
...
def extract_clean_indices(
    ...
    try:
        ...
        indices = [
            ...
            if idx.isdigit() and int(idx) < num_summaries
        ]
        ...
        if logger:
            if len(indices) == count:
                logger.info(
                    f"Case {case_name} (Row {row_index}){chunk_label}: Successfully extracted {count} indices: {indices}"
                )
            else:
                logger.warning(
                    f"Case {case_name} (Row {row_index}){chunk_label}: Extracted {len(indices)} indices instead of {count}: {indices}"
                )
        ...
        if logger:
            logger.error(
                f"Case {case_name} (Row {row_index}){chunk_label}: Error in index extraction: {str(e)}"
            )
        return []
...
def filter_chunk(
    ...
    if len(indices) < count:
        if logger:
            logger.warning(
                f"Case {case_name} (Row {row_index}){chunk_label}: Not enough valid indices extracted. Adding fallback indices."
            )
